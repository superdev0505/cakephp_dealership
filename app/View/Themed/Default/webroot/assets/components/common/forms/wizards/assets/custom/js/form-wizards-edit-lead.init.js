		...
		$(this).bootstrapWizard(
		{
			onNext: function(tab, navigation, index)
			{
				if(index==1){
					...
					else if(wiz.find('#ContactStatus').val()==currentStatus && wiz.find('#ContactActivateLiteVersion').val() == 'Off')
					{
					...
					// if($.trim(wiz.find('#ContactEmail').val()))
					// {
					// 	value=wiz.find('#ContactEmail').val();
				...
				if(index==2){
					...
					if(wiz.find('#ContactSalesStep').val()){
						//Removing per Dan's Request - <?php //https://trello.com/c/mvTOpc2r/909-stock-not-required-on-solds-gainesville-harley-davidson-619 ?>
				...
				if(index==4){
					...
					if(wiz.find('#EventId').val() &&  wiz.find('#EventStatus').val() != 'Completed') {
				...
				if(index==5){
					...
					if(wiz.find('#ContactCreateNewEvent').is(":checked") == false && !wiz.find('#EventId').val()   ) {
			...
			onTabShow: function(tab, navigation, index)
			{
				...
				if(index==4){
					if(wiz.find('#EventId').val() &&  wiz.find('#EventStatus').val() != 'Completed') {
						...
						var btn_label = "Yes, Set Event to Cancel";
						if( wiz.find('#ContactPageRef').val() == 'events' ){
							btn_label = "Yes, Set Event to Complete";
						}

						bootbox.dialog({
							message: "<div  style='text-align: center'><strong>Would you like to clear the event with this log update? </strong> <br> <i style='color: #CC3A3A; font-size: 20px;' class='fa fa-exclamation-triangle'></i> We have a Event scheduled for this customer at <br> ("+wiz.find('#event_starttime_display').val()+" - "+wiz.find('#event_endtime_display').val()+" ).<div>",
							title: "Event log update",
							buttons: {
								success: {
									label: btn_label,
									...
									callback: function() {
										if( wiz.find('#ContactPageRef').val() == 'events' ){
											wiz.find('#EventStatus').val('Completed');
											wiz.find("#EventDetails").val('Completed - by sales person');
										}else{
											wiz.find('#EventStatus').val('Canceled');
											wiz.find("#EventDetails").val('Canceled - by sales person');
										}
									}
								},
								alert: {
									label: "Complete-Set Future event",
									className: "btn-success",
									callback: function() {
						...
						}).find("div.modal-dialog").addClass("largeWidth-min");

						/*
						if(confirm("Would you like to clear the event with this log update ( "+wiz.find('#event_starttime_display').val()+" - "+wiz.find('#event_endtime_display').val()+" ) ? " )){
		...
		wiz.find('.finish').click(function(tab)
		{
			...
			//tab three start
					...
					if(wiz.find('#ContactTypeTrade').val()) {
						...
						if(!wiz.find('#ContactTradeValue').val())
						{
							wiz.find("a[data-toggle*='tab']").eq(2).css('color','red').trigger('click');
							alert('You must select (Trade)Model');
							wiz.find('#ContactModelIdTrade').focus();
			...
			bootbox.dialog({
				...
				buttons: {
					success: {
						...
						callback: function() {
							$("#btn_finish").hide();
							$("#ContactLeadsInputEditForm").submit();
						}
					},
					danger: {
						label: "No",
