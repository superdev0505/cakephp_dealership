...
<div class="row">
	<div class='col-md-9 search_result_paging'>
		<div class="paging">
...
<?php } ?>
	...
	<table class="dynamicTable tableTools table table-bordered checkboxs" id="table-main" style="font-size:12px;width:100%;" border="0" cellpadding="0" cellspacing="0">
		...
		<tr class='bg-inverse' >
			<th>Id</th>
			<th><?php echo  $this->Paginator->sort('company_id', 'Dealer', array('class'=>'search_leads_sort_heads no-ajaxify')); ?></th>
			<th>Customer</th>
			<th>Vehicle</th>
			<th>Source</th>
			<th><?php echo $this->Paginator->sort('created', 'Created', array('class'=>'search_leads_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('modified', 'Modified', array('class'=>'search_leads_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('user_id', 'Sales Person', array('class'=>'search_leads_sort_heads no-ajaxify')); ?></th>
			<th>Originator</th>
			<th><?php echo $this->Paginator->sort('sales_step', 'Step', array('class'=>'search_leads_sort_heads no-ajaxify')); ?></th>

			<?php if($this->request->query['search_bdc_lead'] == '1'){ ?>
			<th>Survey</th>
			<?php } ?>
		...
		<tbody>
			...
			<tr class="gradeA">
				<td class='text-center'>
					...
					<?php if(strrpos($contact['Contact']['company_id'], 'invalid') !== false ) { ?>
						<br><br>
						<span class="label label-danger label-stroke">INVALID - REMOVED</span>
...
<?php //if($this->request->query['search_bdc_lead'] == '1'){ ?>
	...
	<?php if($contact['Contact']['sales_step'] == '10'){ ?>
		<a class='no-ajaxify open_build_ajax btn btn-primary btn-xs' href="/crmgroup/open_build_bdc/<?php echo $contact['Contact']['company_id']; ?>/<?php echo $contact['Contact']['id']; ?>/sold">BDC Work</a>
	<?php }else{ ?>
		<a class='no-ajaxify open_build_ajax btn btn-primary btn-xs' href="/crmgroup/open_build_bdc/<?php echo $contact['Contact']['company_id']; ?>/<?php echo $contact['Contact']['id']; ?>/contact">BDC Work</a>
	<?php } ?>
...
$(function() {
	...
	$(".search_result_paging .paging a,a.search_leads_sort_heads" ).click(function(event){
		...
		$.ajax({
			...
			success: function(results){
				$('#crm_search_result').html(results);
			}
		});
	});
	$("a.search_leads_sort_heads.asc").append(" <i class='fa fa-caret-up'></i>");
	$("a.search_leads_sort_heads.desc").append(" <i class='fa fa-caret-down'></i>");
