...
<div class="row">
		<div class="col-md-8 overdue_events_paging">
			...
			<div>
				<label>
					<input type="radio" value="all" class="btn_overdueevents_color_code" name="overdueevents_colorcode"> <span style="color: #000000">All Lead</span>
				</label> &nbsp;
				<label
					data-toggle="tooltip"
					...
					data-placement="top"
				>
					<input type="radio" value="red" class="btn_overdueevents_color_code" name="overdueevents_colorcode"> <span style="color: #cc3a3a">Unassigned Unworked</span>
				</label> &nbsp;
				...
				<label
					...
					data-placement="top"
				>
					<input type="radio" value="yellow" class="btn_overdueevents_color_code" name="overdueevents_colorcode"> <span style="color: #d2ba02">Assigned Not Worked</span>
				</label> &nbsp;
				...
				<label
					...
					data-placement="top"
				>
					<input type="radio" value="green" class="btn_overdueevents_color_code" name="overdueevents_colorcode"> <span style="color: #8bbf61">Assigned and Worked</span>
				</label>
			</div>
		...
		<div class="col-md-4 text-right" >
			<?php if(!empty($internet_stuffs)){ ?>
					...
					<?php echo $this->Form->input('internet_staff_events', 
	...
	<table class="dynamicTable tableTools table table-bordered checkboxs" id="table-main" style="font-size:12px;width:100%;" border="0" cellpadding="0" cellspacing="0">
		...
		<tr class='bg-inverse' >
			<th>Id</th>
			<th><?php echo  $this->Paginator->sort('company_id', 'Dealer', array('class'=>'overdue_event_sort_heads no-ajaxify')); ?></th>
			<th>Customer</th>
			<th>Vehicle</th>
			<th>Source</th>
			<th><?php echo $this->Paginator->sort('Event.start', 'Event', array('class'=>'overdue_event_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('created', 'Created', array('class'=>'overdue_event_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('modified', 'Modified', array('class'=>'overdue_event_sort_heads no-ajaxify')); ?></th>
			<th>Sales Person</th>
			<th>Owner</th>
			<th><?php echo $this->Paginator->sort('sales_step', 'Step', array('class'=>'overdue_event_sort_heads no-ajaxify')); ?></th>
...
<a class='no-ajaxify open_build_ajax btn btn-primary' href="/crmgroup/open_build_main/<?php echo $contact['Contact']['company_id']; ?>/<?php echo $contact['Contact']['id']; ?>">Work</a>
				...
				<td><?php echo date('m/d/Y g:i A', strtotime($contact['Event']['start'])); ?></td>
...
$(function() {
	...
	?>
		$('input:radio[name="overdueevents_colorcode"]').filter('[value="<?php echo $this->request->query['selected_color']; ?>"]').attr('checked', true);
		selected_color = "<?php echo $this->request->query['selected_color']; ?>";
	<?php  
	}
	?>
	$(".btn_overdueevents_color_code").change(function(){

		selected_color = $("input[type='radio'].btn_overdueevents_color_code:checked").val();
        $.ajax({
			...
			success: function(results){
				$('#web_leads_overdue').html(results);
			}
		});
    });
    //Color code end
	...
	$("#over_due_tab_count").html("<?php echo $this->Paginator->counter("{:count}"); ?>");

	$("#internet_staff_events").change(function(){
		$.ajax({
			url: "/crmgroup/overdue_events",
			type: "get",
			data: {'selected_color' : selected_color,internet_staff:$("#internet_staff_events").val()},
			cache: false,
			success: function(results){ 
				$('#web_leads_overdue').html(results);
			}
	...
	$(".overdue_events_paging .paging a,a.overdue_event_sort_heads" ).click(function(event){
	...
	$("a.overdue_event_sort_heads.asc").append(" <i class='fa fa-caret-up'></i>");
	$("a.overdue_event_sort_heads.desc").append(" <i class='fa fa-caret-down'></i>");
