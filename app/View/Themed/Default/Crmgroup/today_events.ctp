...
<div class="row">
		<div class="col-md-8 today_event_paging">
			...
			<div>
				<label>
					<input type="radio" value="all" class="btn_todaysevent_color_code" name="todaysevents_colorcode"> <span style="color: #000000">All Lead</span>
				</label> &nbsp;
				<label
					data-toggle="tooltip"
					...
					data-placement="top"
				>
					<input type="radio" value="red" class="btn_todaysevent_color_code" name="todaysevents_colorcode"> <span style="color: #cc3a3a">Unassigned Unworked</span>
				</label> &nbsp;
				...
				<label
					...
					data-placement="top"
				>
					<input type="radio" value="yellow" class="btn_todaysevent_color_code" name="todaysevents_colorcode"> <span style="color: #d2ba02">Assigned Not Worked</span>
				</label> &nbsp;
				...
				<label
					...
					data-placement="top"
				>
					<input type="radio" value="green" class="btn_todaysevent_color_code" name="todaysevents_colorcode"> <span style="color: #8bbf61">Assigned and Worked</span>
				</label>
			</div>
		</div>
	...
	<table class="dynamicTable tableTools table table-bordered checkboxs" id="table-main" style="font-size:12px;width:100%;" border="0" cellpadding="0" cellspacing="0">
		...
		<tr class='bg-inverse' >
			<th>Id</th>
			<th><?php echo  $this->Paginator->sort('company_id', 'Dealer', array('class'=>'todays_event_sort_heads no-ajaxify')); ?></th>
			<th>Customer</th>
			<th>Vehicle</th>
			<th>Source</th>
			<th><?php echo $this->Paginator->sort('Event.start', 'Event', array('class'=>'todays_event_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('created', 'Created', array('class'=>'todays_event_sort_heads no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('modified', 'Modified', array('class'=>'todays_event_sort_heads no-ajaxify')); ?></th>
			<th>Sales Person</th>
			<th>Originator</th>
			<th><?php echo $this->Paginator->sort('sales_step', 'Step', array('class'=>'todays_event_sort_heads no-ajaxify')); ?></th>
...
$(function() {
	...
	?>
		$('input:radio[name="todaysevents_colorcode"]').filter('[value="<?php echo $this->request->query['selected_color']; ?>"]').attr('checked', true);
		selected_color = "<?php echo $this->request->query['selected_color']; ?>";
	<?php  
	}
	?>
	$(".btn_todaysevent_color_code").change(function(){

		selected_color = $("input[type='radio'].btn_todaysevent_color_code:checked").val();
        $.ajax({
			...
			success: function(results){
				$('#web_leads_today_event').html(results);
			}
		});
    });
    //Color code end
	...
	$("#today_tab_count").html("<?php echo $this->Paginator->counter("{:count}"); ?>");
	...
	$(".today_event_paging .paging a,a.todays_event_sort_heads" ).click(function(event){
	...
	$("a.todays_event_sort_heads.asc").append(" <i class='fa fa-caret-up'></i>");
	$("a.todays_event_sort_heads.desc").append(" <i class='fa fa-caret-down'></i>");
