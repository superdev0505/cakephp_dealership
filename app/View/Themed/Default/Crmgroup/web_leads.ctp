...
<div class="row">
	<div class="col-md-8 all_weblead_sreach_paging">
		...
		<div>
			<label>
				<input type="radio" value="all" class="btn_allweblead_color_code" name="allweblead_colorcode"> <span style="color: #000000">All Lead</span>
			</label> &nbsp;
			<label
				data-toggle="tooltip"
				...
				data-placement="top"
			>
				<input type="radio" value="red" class="btn_allweblead_color_code" name="allweblead_colorcode"> <span style="color: #cc3a3a">Unassigned Unworked</span>
			</label> &nbsp;
			...
			<label
				...
				data-placement="top"
			>
				<input type="radio" value="yellow" class="btn_allweblead_color_code" name="allweblead_colorcode"> <span style="color: #d2ba02">Assigned Not Worked</span>
			</label> &nbsp;
			...
			<label
				...
				data-placement="top"
			>
				<input type="radio" value="green" class="btn_allweblead_color_code" name="allweblead_colorcode"> <span style="color: #8bbf61">Assigned and Worked</span>
			</label>
		</div>
	</div>
	<div class="col-md-4 text-right" >
		<?php if(!empty($internet_stuffs)){ ?>
				...
				<?php echo $this->Form->input('internet_staff',
	...
	<table class="dynamicTable tableTools table table-bordered checkboxs" id="table-main" style="font-size:12px;width:100%;" border="0" cellpadding="0" cellspacing="0">
		...
		<tr class='bg-inverse' >
			<th>Id</th>
			<th><?php echo  $this->Paginator->sort('company_id', 'Dealer', array('class'=>'lead_pushed_to_me_sorthead no-ajaxify')); ?></th>
			<th>Customer</th>
			<th>Vehicle</th>
			<th>Source</th>
			<th><?php echo $this->Paginator->sort('created', 'Created', array('class'=>'lead_pushed_to_me_sorthead no-ajaxify')); ?></th>
			<th><?php echo $this->Paginator->sort('modified', 'Modified', array('class'=>'lead_pushed_to_me_sorthead no-ajaxify')); ?></th>
			<th>Sales Person</th>
			<th>Originator</th>
			<th><?php echo $this->Paginator->sort('sales_step', 'Step', array('class'=>'lead_pushed_to_me_sorthead no-ajaxify')); ?></th>
...
echo $this->element("sql_dump");
	...
	// debug($this->request->query['selected_color']);
...
$(function() {
	...
	?>
		$('input:radio[name="allweblead_colorcode"]').filter('[value="<?php echo $this->request->query['selected_color']; ?>"]').attr('checked', true);
		selected_color = "<?php echo $this->request->query['selected_color']; ?>";
	<?php  
	}
	?>
	$(".btn_allweblead_color_code").change(function(){

		selected_color = $("input[type='radio'].btn_allweblead_color_code:checked").val();
		window.crmgroup_refresh_url = "/crmgroup/web_leads?selected_color="+selected_color;
        $.ajax({
			url: "/crmgroup/web_leads",
			type: "get",
			data : {'selected_color' : selected_color},
			cache: false,
			success: function(results){
				$('#weblead_result').html(results);
			}
		});
    });
    //Color code end
	...
	$("#all_web_tab_count").html("<?php echo $this->Paginator->counter("{:count}"); ?>");


	$("#internet_staff").change(function(){

		$.ajax({
			url: "/crmgroup/web_leads",
			type: "get",
			data: {'selected_color' : selected_color, dealer_id: "", internet_staff:$("#internet_staff").val()},
			cache: false,
			success: function(results){
				$('#weblead_result').html(results);
			}
	...
	$(".all_weblead_sreach_paging .paging a, a.lead_pushed_to_me_sorthead" ).click(function(event){
	...
	$("a.lead_pushed_to_me_sorthead.asc").append(" <i class='fa fa-caret-up'></i>");
	$("a.lead_pushed_to_me_sorthead.desc").append(" <i class='fa fa-caret-down'></i>");
