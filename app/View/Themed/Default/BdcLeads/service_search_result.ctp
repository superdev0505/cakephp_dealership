...
$count = 0;	foreach ($contacts['new'] as $contact): 	?>
	<div class="col-app  list_search_result border-bottom"  lead_row_id = "<?php echo $contact['ServiceLeadsDms']['id'];  ?>" style="cursor: pointer">
		...
		<div class="media innerAll">
			...
			<div class="media-body">
				<span class="strong muted">
					<font color="blue"><u><?php echo $contact['ServiceLeadsDms']['name'] ;  ?></u></font>
					<?php			
						echo '<span class="label label-info label-stroke">Service Lead</span>';
								
					?>&nbsp; #<?php echo $contact['ServiceLeadsDms']['id']; ?>
				</span>
			</div>
			...
			<span class="muted">
				<i class="fa fa-mobile"></i>
					...
					echo $cleaned;?>
					&nbsp;<i class="fa fa-home"></i>&nbsp;<?php echo $contact['ServiceLeadsDms']['city']; ?> 
			</span>	

			<span class="muted">
				 <?php echo $contact['ServiceLeadsDms']['unit_year']; ?>  &nbsp;<?php echo $contact['ServiceLeadsDms']['unit_make']; ?> &nbsp;<?php echo $contact['ServiceLeadsDms']['unit_model']; ?>
			</span>
			...
			<span class="muted">
				<i class="fa fa-calendar"></i> <?php echo date('D -M d,Y h:i A',strtotime($contact['ServiceLeadsDms']['modified'])); ?>
			</span>
			
			
		
			
            <span><strong>Service writer:</strong>&nbsp;<?php echo $contact['ServiceLeadsDms']['service_writer'] ?>&nbsp;</span>
															<br /><span><strong>Comment:</strong>&nbsp;<?php echo $contact['ServiceLeadsDms']['end_customer'] ?>&nbsp;</span> 
...
<?php $count = 0;	foreach ($contacts['contact'] as $contact): 	?>
	<div class="col-app  list_search_result border-bottom"  lead_row_id = "<?php echo $contact['ServiceLeadsDms']['id'];  ?>" style="cursor: pointer">
		...
		<div class="media innerAll">
			...
			<span class="muted">
				<?php echo $contact['BdcLead']['condition']; ?> &nbsp; <?php echo $contact['ServiceLeadsDms']['year']; ?>  &nbsp;<?php echo $contact['ServiceLeadsDms']['unit_make']; ?> &nbsp;<?php echo $contact['ServiceLeadsDms']['unit_model']; ?>
			</span>
			 >
			<span>
			<strong>Dormant: <?php 
...
$(function() {
	...
	$id='';
		...
		{
			$id=$contacts['new'][0]['ServiceLeadsDms']['id'];
		}
		elseif(!empty($contacts['contact']))
		 {
			 $id=$contacts['contact'][0]['ServiceLeadsDms']['id'];  
		 }
		 elseif(!empty($contacts['no_contact']))
		 {
			 $id=$contacts['no_contact'][0]['ServiceLeadsDms']['id'];  
		 }
		 elseif(!empty($contacts['bad_number']))
		 {
			 $id=$contacts['bad_number'][0]['ServiceLeadsDms']['id'];  
		 }
		 elseif(!empty($contacts['voicemail']))
		 {
			 $id=$contacts['voicemail'][0]['ServiceLeadsDms']['id'];  
		 }
		 elseif(!empty($contacts['call_back']))
		 {
			 $id=$contacts['call_back'][0]['ServiceLeadsDms']['id'];  
		 }
	...
	$.ajax({
		...
		echo $this->Html->url(array('action' => 'service_lead_details',$id)); ?>",
		success: function(data){
    ...
    $(".list_search_result").click(function() {
		...
		$.ajax({
			type: "GET",
			cache: false,
			url:  "<?php echo $this->Html->url(array('action' => 'service_lead_details', 'selected_lead_type'=>$selected_lead_type)); ?>/"+$(this).attr('lead_row_id'),
			success: function(data){
				$("#lead_details_content").html(data);
				$("#lead_details_content").prev('.ajax-loading').addClass('hide');
			}
