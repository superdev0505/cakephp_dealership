					...
					<div class="widget-body"> 
                        ...
                        <?php echo $this->Form->hidden('BdcLead.id',array('value'=>$id)); ?>
                                ...
                                <div class="col-md-10">
									...
									<div class="col-md-10">
										<label>Hello, I’m calling to see if <i style="font-size: 14px;"><?php echo  $contact_info['BdcLead']['full_name']; ?></i> is available?  Hello <?php echo $fname.' '.$lname; ?>.  My name is <i style="font-size: 14px;"><?php echo  $sperson; ?></i>and I’m calling on behalf of <i style="font-size: 14px;"><?php echo $dealer; ?></i> here in Chattanooga. Did I catch you at a good time?</label>
										<div class="separator"></div>
									</div>
									<div class="col-md-10">
										<label>	
                                        We are entering a new phase here at the store and trying to clean up our database so I had just a couple of survey questions to ask you. May-I </label>
										<div class="separator"></div>
									</div>
									<div class="col-md-10">
										<label>1. Confirm that you still own that <?php echo $contact_info['BdcLead']['year'].' '.$contact_info['BdcLead']['make'].' '.$contact_info['BdcLead']['model']; ?> ?
										<?php echo $this->Form->input('SurveyAnswer.138.answer', array('options' => array('Yes'=>'Yes', 'No'=>'No'), 'type' => 'select', 'class' => 'form-control','empty'=>'Select'));
										echo $this->Form->hidden('SurveyAnswer.138.question_id',array('value'=>'138'));
										echo $this->Form->hidden('SurveyAnswer.138.bdc_survey_id',array('value'=>''));
										
										?><br/>
                                      a. If yes: Fantastic – are you getting in as much riding time as you hope to? By the way, when did you purchase your bike? 
                                      <br/>
                                      b. If no: I’ll make a note of that, did you trade for something else or did you just lose interest in riding?
                                       </label> 
                                      <?php 
									   echo $this->Form->input('138.1', array('type' => 'textarea','class' => 'form-control novalidate','value'=>''));
									  ?>
										<div class="separator"></div>
									</div>
									<div class="col-md-10 no-show-div">
										<label>2.  Confirm that we have correct contact info on file, I have your current address as: <?php echo $contact_info['BdcLead']['address'].' '.$contact_info['BdcLead']['city'].' '.$contact_info['BdcLead']['state']; ?> And your current email as: <?php echo $contact_info['BdcLead']['email']; ?>  
And is this number the best number for us to use in the future if we need to make contact?</label>
										<?php echo $this->Form->input('SurveyAnswer.139.answer', array('type' => 'textarea','class' => 'form-control','value'=>''));
										echo $this->Form->hidden('SurveyAnswer.139.question_id',array('value'=>'139'));
										echo $this->Form->hidden('SurveyAnswer.139.bdc_survey_id',array('value'=>''));
										?>
                                        <div class="separator"></div>
                                        </div>
                                        <div class="col-md-10 no-show-div">
										<label>3. If the customer is local to Chattanooga then ask: Have you had a chance to visit the new location on the corner of Hwy 58 and 153 yet?</label>
										<?php 
										echo $this->Form->input('SurveyAnswer.140.answer', array('options' => array('Yes'=>'Yes', 'No'=>'No'), 'type' => 'select', 'class' => 'form-control','empty'=>'Select'));
										?>
                                        <br/>
                                        a. If yes: What do you think about it? <br/>
                                        b. If no: I’d love to personally invite you to stop by soon just to check out what’s new in the <?php echo $contact_info['BdcLead']['make']; ?> lineup and see some of the new accessories we’re now offering. Tripling our showroom size has been an amazing benefit and allowed us to increase inventory and offer new lines of Helmets, riding gear, etc.
                                        <?php
										echo $this->Form->input('140.1', array('type' => 'textarea','class' => 'form-control','value'=>'')); 
										echo $this->Form->hidden('SurveyAnswer.140.question_id',array('value'=>'140'));
										echo $this->Form->hidden('SurveyAnswer.140.bdc_survey_id',array('value'=>''));
										?>
                                       
										<div class="separator"></div>
									</div>
									<div class="col-md-10 no-show-div">
										<label>4.  If the customer has owned their current bike longer than 6 months: We find ourselves now more than ever making top dollar cash offers for local bikes. Would you allow us to make a cash “buy back” or “trade in” offer on your Model – at your first convenience, of course?</label>
										<?php echo $this->Form->input('SurveyAnswer.151.answer', array('options' => array('Yes'=>'Yes', 'No'=>'No'), 'type' => 'select', 'class' => 'form-control','empty'=>'Select'));
										echo $this->Form->hidden('SurveyAnswer.151.question_id',array('value'=>'151'));
										echo $this->Form->hidden('SurveyAnswer.151.bdc_survey_id',array('value'=>''));
										?>
										<div class="separator"></div>
									</div>
									<div class="col-md-10 no-show-div">
										<label>5. If yes: Great, do you prefer something thru the week or maybe Saturday?</label>
										<?php echo $this->Form->input('SurveyAnswer.152.answer', array('type' => 'textarea','class' => 'form-control','value'=>'')); 
										echo $this->Form->hidden('SurveyAnswer.152.question_id',array('value'=>'152'));
										echo $this->Form->hidden('SurveyAnswer.152.bdc_survey_id',array('value'=>''));
										?>
                                        <br/>
                                        <label>If no: I’ll make a note that you’re loving what you’re currently riding and try to stay in touch with you every few months.</label>
										<div class="separator"></div>
									</div>
									...
									<div class="col-md-10">
                                        ...
                                        Thanks so much for taking my call today. Ride well!!!</label>
										<div class="separator"></div>
									</div>
                                    <div class="col-md-10">
...
$(function(){
	...
	$("#SurveyAnswer143Answer").on('change',function(){
		$val = $(this).val();
		if($val == 'Yes')
		{
	...
	$("#SurveyAnswer138Answer").on('change',function(){
		$val = $(this).val();
		/*if($val == 'Yes')
		{
			$(".no-show-div").show();
			$("#SurveyAnswer139Answer").removeClass('novalidate');
			$("#SurveyAnswer140Answer").removeClass('novalidate');
			$("#SurveyAnswer141Answer").removeClass('novalidate');
			$("#SurveyAnswer142Answer").removeClass('novalidate');
			
		}else
		{
			$(".no-show-div").hide();
			$("#SurveyAnswer139Answer").addClass('novalidate');
			$("#SurveyAnswer140Answer").addClass('novalidate');
			$("#SurveyAnswer141Answer").addClass('novalidate');
			$("#SurveyAnswer142Answer").addClass('novalidate');
		}*/
...
function submit_s_form()
{
  ...
  success: function(){
		   ...
		   $.ajax({
				  type: "GET",
				  cache: false,
				  url:  "/bdc_leads/equity_search_result/load_first/equity_search_id:"+$("#EquitySearchId").val(),
				  success: function(data){
					  bootbox.hideAll();
