...
echo $this->Html->script('jquery.noty.packaged.min');
 ?>
<style>
...
elseif($lead_type=='equity_search')
{
	$form_action='equity_search_result';
}
?>
<div class="innerLR" id="BdcMainDiv">
  ...
  <div class="widget" style="margin-bottom:0px;">
		...
		<div class="row  row-merge margin-none " style="padding:3px;">
			...
			if(isset($this->params['bdcapp']) && isset($build_ids[$d_id])){
			?>
              <h2 style="text-align:left;font-size:14px;margin-left:5px;"><strong><?php echo $build_ids[$d_id];
			  if($d_id != 'all_builds'){
			  echo ' - # '.$d_id;
			  }
			   ?></strong></h2>
		...
		<div class="row row-merge margin-none ">
				...
				<div class="widget-body">
					...
					<div class="row">
						...
						if($lead_type==''){
						 ...
						 echo $this->Form->create('BdcLead', array('url' => array('controller'=>'bdc_leads','action' => $form_action,'load_first') ,'autocomplete'=>"off", 'class' => 'form-inline no-ajaxify','id' =>'BdcLeadSearchResultForm')); ?>
						...
						if(in_array(AuthComponent::user('dealer_id'),array(5000,20080,20090,20070)))
						{$main_lead_bdc_type = array(
								'contact' => 'Pre-Purchase',
								'sold' => 'CSI Sold',
								/*'service' => 'Service Leads',
						...
						}elseif(in_array($dealer_id,$freedom_group))
						{
							$main_lead_bdc_type = array(
								...
								'sold' => 'CSI Sold',
								'service' => 'Service Leads',
								'equity_search' => 'Equity Calls'
							);
						}
						elseif(in_array($dealer_id,$ridenow_group))
						{
							$main_lead_bdc_type = array(
								'contact' => 'Pre-Purchase',
								'sold' => 'CSI Sold',
                       ...
                       <a href="#" class="btn btn-primary pull-left no-ajaxify" id="toggle_sort" data-id="DESC">New- Lead View</a></div><?php */?>
						...
						<div class="col-md-3" style="width:31% !important">

                        <a href="javascript:void(0)" id ="reset_all_search" style="margin-left:10px;" class="btn btn-inverse pull-right">Reset</a>
                        ...
                        <div class="overflow-hidden">
                            <div class="input-group date" ><span class="input-group-addon"><i class="fa fa-th"></i></span>
								<?php
								 echo $this->Form->input('search_all2', array('style' => "",'label'=>false,'div'=>false ,'type' => 'text', 'class' => 'form-control', 'required' => false,'placeholder'=>'Start date - Search','style' =>'width:47%;margin-right:2%'));
								echo $this->Form->input('end_date_search', array(
								'div' => false,
								'class' => 'form-control',
								'label' => false,
								'style' =>'width:47%;',
								'placeholder'=>'End date - Search'
								));
                     ...
                     <button type="submit" id="btn-submit-search2" class="btn btn-info pull-right no-ajaxify"><i class="icon-search"></i></button>
                        <div class="overflow-hidden">
                         <?php echo $this->Form->input('search_all_value', array('style' => "",'label'=>false,'div'=>false ,'type' => 'text', 'class' => 'form-control pull-right', 'required' => false,'placeholder'=>'Search','style'=>'width:80%')); ?>
                         </div>
                        </div>
                        ...
                        <div class="overflow-hidden">
                          ...
                          <?php }elseif($lead_type == 'equity_search'){ ?>
                     ...
                     <button type="submit" id="btn-submit-search2" class="btn btn-info pull-right no-ajaxify"><i class="icon-search"></i></button>
                        ...
                        <div class="overflow-hidden">
							<?php
							$search_value = '';
							if(isset($this->params['named']['equity_search_id']))
							{
								$search_value=$this->params['named']['equity_search_id'];
							}

							echo $this->Form->select('equity_search_id', $equity_searches, array('div' => false, 'value'=>$search_value, 'class'=>"form-control", 'style' => 'width: 100%', 'id' => 'EquitySearchId'));
							//keep the state of Quick Search
							//echo $this->Form->hidden("search_all_value", array('value' => $selected_type));
							?>
                            </div>
            ...
            <div class="col-md-3" style="width:25% !important">
              ...
              <div class="overflow-hidden">
                <div class="input-group date" ><span class="input-group-addon"><i class="fa fa-th"></i></span>
                <?php
                  echo $this->Form->input('start_date_search', array(
                    'label'=>false,
                    'div'=>false ,
                    'type' => 'text',
                    'id' => 'BdcLeadSearchAll2',
                    'class' => 'form-control',
                    'required' => false,
...
$script.ready('bundle', function() {
	...
	$("#BdcLeadEndDateSearch").bdatepicker({
			format: 'yyyy-mm-dd',
			endDate: "<?php echo date('Y-m-d'); ?>"
		});
	...
	$("#form_reset_button").click(function() {
		...
		$('body').on('click',"#toggle_sort",function(e){
			...
			var search_url = $("#BdcLeadSearchResultForm").attr('action') + "/search_all2:"+$("#BdcLeadSearchAll2").val()+"/search_all:"+$("#ContactSearchAll").val()+"/search_all_value:/selected_lead_type:<?php echo $selected_lead_type; ?>/l_sort:"+sort_order+"/search_last_date:"+$("#BdcLeadEndDateSearch").val()+"/selected_lead_type:<?php echo $selected_lead_type; ?>";
			$.ajax({
				type: "GET",
				cache: false,
				url:  search_url,
		...
		$("#btn-submit-search").click(function(event){
			...
			if($("#BdcLeadSearchAll2").val() == '')
			{
				alert('Please select start and end search date');
				return false;
			}
			if($("#BdcLeadEndDateSearch").val() == '')
			{
				alert('Please select end search date');
				return false;
			}

			$("#search-result-content").html("");
			$("#lead_details_content").html("");
			$("#ContactSearchAll").val('');
			$("#BdcLeadSearchAllValue").val('');
			$('.ajax-loading').removeClass('hide');
			event.preventDefault();
			var search_url =  "<?php echo $this->Html->url(array('controller' => 'bdc_leads','action' => 'search_result')); ?>/search_all2:"+$("#BdcLeadSearchAll2").val()+"/search_all:"+$("#ContactSearchAll").val()+"/search_last_date:"+$("#BdcLeadEndDateSearch").val()+"/selected_lead_type:<?php echo $selected_lead_type; ?>";
			$.ajax({
				type: "GET",
				cache: false,
				url:  search_url,
		...
		$("#ContactSearchAll").on('change',function(){
						if($(this).val()!='')
						{
							$("#BdcLeadSearchAll2").val('');
							$("#BdcLeadSearchAllValue").val('');
							$("#BdcLeadEndDateSearch").val('');
							$("#lead_details_content").html("&nbsp;")
							$("#search-result-content").html("");
							$('.ajax-loading').removeClass('hide');
							//	event.preventDefault();
		...
		$("#btn-submit-search2").click(function(event){
			...
			if($.trim($("#BdcLeadSearchAllValue").val()) == ''){
	...
	$("#ContactSearchAll").on('change',function(){
		...
		$("#btn-submit-search2").click(function(event){
			...
			$('.ajax-loading').removeClass('hide');
			event.preventDefault();
			var search_url =  "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'event_sold_result','load_first','lead_action' =>$lead_action )); ?>/view_lead_id:"+$("#BdcLeadSearchAllValue").val();
			$.ajax({
				type: "GET",
				cache: false,
				url:  search_url,
	...
	<?php }elseif($lead_type == 'equity_search'){  //Andi
		$lead_action = $lead_type
		?>
	...
	$("#lead_details_content").html("&nbsp;");
		$('.ajax-loading').removeClass('hide');
		$url='<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'equity_search_result','load_first')); ?>/equity_search_id:'+$equity_search_id+$view_lead_id;
		$.ajax({
			type: "GET",
			cache: false,
			url:  $url,
	...
	$("#EquitySearchId").on('change',function(){
						if($(this).val()!='')
						{
							$("#BdcLeadSearchAll2").val('');
							$("#BdcLeadSearchAllValue").val('');
							...
							$('.ajax-loading').removeClass('hide');
							//	event.preventDefault();
							var search_url = "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'equity_search_result','load_first' )); ?>/equity_search_id:"+$("#EquitySearchId").val();
							$.ajax({
								type: "GET",
								cache: false,
								url:  search_url,
		...
		$("#btn-submit-search2").click(function(event){
			...
			$('.ajax-loading').removeClass('hide');
			event.preventDefault();
			var search_url =  "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'equity_search_result','load_first' )); ?>/serach_keyword:"+$("#BdcLeadSearchAllValue").val()+"/equity_search_id:"+$("#EquitySearchId").val();
			$.ajax({
				type: "GET",
				cache: false,
				url:  search_url,
    ...
    $("#btn-submit-search").click(function(event){
      ...
      $('.ajax-loading').removeClass('hide');
      event.preventDefault();
      var search_url =  "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'equity_search_result','load_first' )); ?>/serach_keyword:"+$("#BdcLeadSearchAllValue").val()+"/equity_search_id:"+$("#EquitySearchId").val()+"/search_all2:"+$("#BdcLeadSearchAll2").val()+"/search_last_date:"+$("#BdcLeadEndDateSearch").val();
  ...
  //console.log('<?php echo $lead_type; ?>');
	...
	$("#reset_all_search").on('click',function(e){
			e.preventDefault();
			$("#BdcLeadSearchAllValue").val('');
			$("#BdcLeadSearchAll2").val('');
			$("#ContactSearchAll").val('');
...
<script type="text/javascript">
   function generate_noty(type, text) {
	...
	function get_callback_info($survey_id)
	{
		$.ajax({
			type : 'GET',
			url: '<?php echo $this->Html->url(array('controller' => 'bdc_leads','action' => 'get_callback_info')); ?>/'+$survey_id,
			success: function(data){
				if(data){
				 generate_noty('information', data);
				}
			}
			});
	}
	...
	setTimeout(function(){get_callback_info(2);},20000);
	setTimeout(function(){get_callback_info(3);},30000);
	setInterval(function(){ get_callback_info(2); }, 36000000);
	setInterval(function(){ get_callback_info(3); }, 36001000);
</script>
