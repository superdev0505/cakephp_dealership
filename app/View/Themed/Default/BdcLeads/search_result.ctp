 <span class="results" style="height:23px;font-size:15px;"><?php echo $contacts_count; ?> Leads Found <i class="icon-circle-arrow-down"></i> &nbsp; | &nbsp; <a href="#"  id="toggle_sort" style="text-decoration:underline;" data-id="<?php echo $l_sort; ?>">
 <?php echo ($l_sort=='ASC')?'Past- Lead View':'New- Lead View'; ?>
 </a></span>
		...
		<div class="media innerAll">
            ...
            <span><strong>Sales Person:</strong>&nbsp;<?php echo $contact['BdcLead']['sperson'] ?>&nbsp;</span>
				...
				$contact['BdcLead']['id'].'" >Read more</a>'));	 ?>&nbsp;</span> 
                 <?php if (isset($bdc_status_leads[$contact['BdcLead']['id']]) && $is_sold_leads){ ?>
               <br /> <span style="font-size:12px;"><strong><i class="fa fa-certificate" style="color:#F00;"></i>&nbsp;<i>Pre-Purchase survey has been completed please now attempt the Sold CSI Survey.</i></strong>&nbsp;<?php //echo $bdc_status_leads[$contact['BdcLead']['id']]; ?>&nbsp;</span><br/>
			   <?php } ?>  
...
<?php $count = 0;	foreach ($contacts['call_back'] as $contact): 	?>
    ...
    <?php if(!empty($contact['BdcSurvey']['call_back_date'])) {?>
	 <span><strong>CallBack Date: </strong>&nbsp;<?php echo date('D - M d,Y h:i A',strtotime($contact['BdcSurvey']['call_back_date'])); ?>&nbsp;</span>
<?php }
...
<?php $count = 0;	foreach ($contacts['voicemail'] as $contact): 	?>
	<div class="col-app  list_search_result border-bottom"  lead_row_id = "<?php echo $contact['BdcLead']['id'];  ?>" style="cursor: pointer">
		...
		<div class="media innerAll">
			...
			<?php } ?> 
			 <span><strong>Sales Person:</strong>&nbsp;<?php echo $contact['BdcLead']['sperson'] ?>&nbsp;</span>
															<br /><span><strong>Comment:</strong>&nbsp;<?php echo $contact['BdcLead']['notes'] ?>&nbsp;</span>
...
$(function() {
	...
	$(".lnk_merge_all_matched").click(function(event){
		...
		$.ajax({
			type: "GET",
			cache: false,
			url:  "<?php echo $this->Html->url(array('controller' => 'contacts_manage','action' => 'merge_all_matched')); ?>/"+$(this).attr('contact_id'),
			success: function(data){
    ...
    $(".list_search_result").click(function() {
		...
		$.ajax({
			type: "GET",
			cache: false,
			url:  "<?php echo $this->Html->url(array('controller' => 'bdc_leads','action' => 'lead_details', 'selected_lead_type'=>$selected_lead_type,'event_solds' => $event_lead_type)); ?>/"+$(this).attr('lead_row_id'),
			success: function(data){
				$("#lead_details_content").html(data);
				$("#lead_details_content").prev('.ajax-loading').addClass('hide');
			}
