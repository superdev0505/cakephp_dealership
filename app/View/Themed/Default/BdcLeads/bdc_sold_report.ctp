...
<div class="innerLR">
	...
	//echo $this->Html->link('<i class="fa fa-bar-chart-o"></i> Group Stats',array(),array('class'=>'btn btn-inverse pull-right','id'=>'LeadGroupStat','escape'=>false,'style'=>'display:none;','data-id'=>'dealership')); 
		...
		<i class="fa fa-bar-chart-o"></i> BDC/Sales Appts Sold Report
...
&nbsp;&nbsp;&nbsp;
		...
		<?php 
			...
			echo $this->Form->select('user_id', $stat_options, array('value'=>$selected_stats,'style'=>"display: inline-block;width: auto;")); 
...
$script.ready('bundle', function() {
	...
	$("#user_id").change(function(){
			...
			if($(this).val() != ""){
				var url = "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'sold_report_info')); ?>/"+$("#user_id").val()+"/"+$("#startDate").val()+"/"+$("#endDate").val();
				if(url!=''){
					$.ajax({
						type: "POST",
						cache: false,
      ...
      //  console.log(start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'), label);
		...
		if($("#select_report").val()!=''){
			$.ajax({
				type: "GET",
				cache: false,
				url: "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'sold_report_info')); ?>/"+ $("#user_id").val()+"/" + start.format('YYYY-MM-DD')+"/"+ end.format('YYYY-MM-DD'),
				success: function(data){
