                    ...
                    <?php //echo $this->element('other_location_bdc_leads'); ?>
                        ...
                        <?php echo $this->Form->hidden('BdcLead.id',array('value'=>$id)); ?>
                            ...
                            <div class="col-md-10">
								...
								echo $this->Form->hidden('BdcSurvey.survey_id',array('value'=>14));
                           ...
                           <div class="col-md-10" id="VoicemailScript" style="display:none">
                                ...
                                <label>  Hi <i style="font-size: 14px;color:#4193d0;"><?php echo  $contact_info['BdcLead']['full_name']; ?></i>  my name is <i style="font-size: 14px;color:#4193d0;"><?php echo  $sperson; ?></i> and I am calling on behalf of

 <i style="font-size: 14px;color:#4193d0;"><?php echo $dealer; ?></i> . <br /><br />



The reason for my call is to invite you to a special event and swap meet the dealership is having for their very important customers, friends and family.
              <br /><br />
              Beginning June 15thrunning through Father’s Day, the dealership is offering 20% off most items in Motorclothes. 11% off all parts and accessories, and a$1,500 shopping spree with the purchase of any, in stock, NEW 2015 Big Twin and $1,000 shopping spree with the purchase of any, in stock, new 2015 Sportster. Please note, this is for in stock units ONLY and the Sale ends June 20th.<br />
             The swap meet is being held Saturday the 20th. However, it’s FREE to have a booth! All you need to do is call the dealership and ask for Neil.<br />
       If you have any other questions or would just like more information on the event, please call the dealership directly at (507) 454-4578.
</label>
 ...
 <hr />
									...
									<div class="col-md-10">
									<label>	I am calling to see if <i style="font-size: 14px;"><?php echo  $contact_info['BdcLead']['full_name']; ?></i> is available?</label> <?php /*echo $this->Form->input('SurveyAnswer.18.answer', array('type' => 'select', 'options' => array(
											'Yes' => 'Yes',
											'No' => 'No'
									...
									<div class="col-md-10">
										...
										calling on behalf of <i style="font-size: 14px;"><?php echo $dealer; ?></i> customer service department. how are you today? </label>             
              <br />
              <label>Hi, the reason for my call is the dealership is hosting a special event for their very important customers. They would like to invite you as well as your friends and family to attend. The Dealership is offering a $1,500 shopping spree with the purchase of a NEW 2015 Big Twin and $1,000 on the purchase of a new 2015 Sportster! </label>   
									...
									<div class="col-md-10">
										<label> The event starts today and only runs through May 2nd.Can I mark you as attending?
										<?php echo $this->Form->input('SurveyAnswer.97.answer', array('type' => 'select','class' => 'form-control','options' => array('n/a' => 'N/A','yes' => 'Yes','no' => 'No','maybe' => 'Maybe'),'empty'=>'select')); 
										...
										<?php echo $this->Form->input('SurveyAnswer.99.answer', array('options' => array('n/a' => 'N/A','yes' => 'Yes','no' => 'No','maybe' => 'Maybe'), 'type' => 'select', 'class' => 'form-control'));?>
                                         <br />Dealerships address is:  1116 Oak Forest Dr, Onalaska, WI 54650
                                        <br />Their phone number is: (608) 783-6112
                                        
                                        </label>
                                        <?php
										echo $this->Form->input('99.1', array('type' => 'textarea','class' => 'form-control novalidate'));
										echo $this->Form->hidden('SurveyAnswer.99.question_id',array('value'=>'99'));
										echo $this->Form->hidden('SurveyAnswer.99.bdc_survey_id',array('value'=>''));
									...
									<div class="col-md-10">
										<label>	If you have any questions, you can call the dealership directly at (608) 783-6112 </label>
										<div class="separator"></div>
									</div>
                                    <div class="col-md-10">
...
$(function(){
	...
	$(".BdcInvalidLead").on('click',function(e){
		...
		$.ajax({
			...
			success: function(data){
				...
				type: "GET",
				cache: false,
				url: "<?php echo $this->Html->url(array('controller'=>'bdc_leads','action'=>'lead_details','event_solds' => 'event_leads')); ?>/"+next_id,
				success: function(data){
	...
	$("a#skip_lead").click(function(event)
	{
		...
		if(next_id)
		{
			$.ajax({
					type: "GET",
					cache: false,
					url: "/bdc_leads/dealer_event_survey/"+next_id+"/14",
					success: function(data){
	...
	$("a#btn-cancel-survey").click(function(event){
		...
		$.ajax({
			type: "GET",
			cache: false,
			url: "/bdc_leads/lead_details/event_solds:event_leads/selected_lead_type:/<?php echo $this->request->params['pass']['0'] ?>",
			success: function(data){
				bootbox.hideAll();
				$("#lead_details_content").html(data);
			}
			...
			success: function(){
					...
					success: function(data){
						...
						$.ajax({
							type: "GET",
							cache: false,
							url:  "/bdc_leads/event_sold_result/lead_action:event_leads/search_all2:"+search_all2+"/search_all:"+search_all+"/search_all_value:/selected_lead_type",
							success: function(data){
	...
	$("#BdcSurveyDuration").val(duration);
		$.ajax({
			...
			success: function(){
					...
					type: "GET",
					cache: false,
					url: "/bdc_leads/event_sold_result/lead_action:event_leads/load_first/search_all2:"+search_all2+"/search_all:"+search_all+"/search_all_value:/selected_lead_type",
					success: function(data){
						bootbox.hideAll();
						$("#search-result-content").html(data);
						$('.ajax-loading').addClass('hide');
