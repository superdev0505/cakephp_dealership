<?php if(!empty($err_msg)){ $this->Session->setFlash(__('Only the creator of this Search can delete it.'), 'alert', array('class' => 'alert-error')); } ?>
...
data-searchid="<?php echo $equitySearch['EquitySearch']['id']; ?>" class="label label-success label-stroke">
	...
	&nbsp; &nbsp;
	<a class="remove_equity_searches" href="#" onclick="remove_equity_search('<?php echo $equitySearch['EquitySearch']['id']; ?>')"   class="btn btn-xs btn-danger">X</a>
</span>
