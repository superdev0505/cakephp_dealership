			...
			<div class="widget widget-heading-simple widget-body-white">
				...
				<div class="widget-body">
						...
						<div style="">
							...
							<?php echo $this->Form->create('Contact', array('url'=>array('controller'=>'rules'
              , 'action'=>'save_refnum_rule/'.strtolower($keyword))
              ,'id'=>'Contact'.ucfirst($keyword).'Form'
              , 'type'=>'post', 'class' => 'apply-nolazy', 'role'=>"form")); ?>
							...
							<div class="checkbox">
								...
								<?php echo $this->Form->select($keyword.'_users_id', $users, array('value' => $rule_direct['Rule']['user_id'], 'class' => 'input-small','empty'=>"Select")); ?>
							</div>
...
echo $this->Form->select('roundrobin_novehicle_'.$keyword,$all_active_users, array('vendor'=>$keyword,'value'=>$RulesRoundrobinNovehicle_user_id, 'class' => 'input-small roundrobin_novehicle','empty'=>"Select"));
            ...
            <table class="table table-striped table-responsive swipe-horizontal ">
                            ...
                            <tbody>
                                <!-- Table row -->
<?php echo $this->element("queue_table", array('weblead_queue_list' => ${$keyword .'_weblead_queue'}, 'tb_model_name' => 'WebleadQueue')); ?>
                                <!-- // Table row END -->
                            </tbody>
                            <!-- // Table body END -->
                        </table>

									<div>Next Receive: <span class="text-primary"><?php echo ${$keyword .'_next_receive'}['User']['full_name']; ?> # <?php echo ${$keyword .'_next_receive'}['User']['id']; ?></span></div>
