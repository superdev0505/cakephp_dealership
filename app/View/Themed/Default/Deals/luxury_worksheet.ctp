...
$selected_months=array('1'=>'48','2'=>'60','3'=>'72');
?>
    ...
    <?php } ?>
		<input name="contact_id" type="hidden"  value="<?php echo $info['contact_id']; ?>" />
            ...
            <img class="border-none" style="width:49.1%;height:31px;background:#000000;padding:0px;"/>
            ...
            <span class="span24">Selling Price</span>$
				...
				<input name="trade_allowance" id="trade_allowance" type="text" class="input71 priceChange" value="<?php echo $info['trade_value']; ?>" placeholder=""/>
                <span class="span24">Trade Payoff</span>$
				<input name="trade_payoff2" id="trade_payoff" type="text" class="input71 priceChange" value="<?php echo $info['trade_payoff']; ?>" placeholder=""/>
                <span class="span24">Cash Down</span>$
				<input name="cash_down" id="cash_down" type="text" class="input71 priceChange" value="<?php echo $info['cash_down']; ?>" placeholder=""/>
                <span class="span24">Doc Fee</span>$
				<input name="doc_fee" id="doc" type="text" class="input71 priceChange" value="<?php echo $info['doc_fee']; ?>" placeholder=""/>
                <span class="span24"> Title Fee</span>$
				<input name="tag_fee" id="tag_tit"type="text" class="input71 priceChange" value="" placeholder=""/>
                <span class="span24">Registration Fee</span>$
				<input name="licreg_fee" id="licreg_fee" type="text" class="input71 priceChange" value="<?php echo $info['licreg_fee']; ?>" placeholder=""/>
                ...
                <span class="span24">GAP Insurance</span>$
				<input name="gap_fee"  id="gap_fee" type="text" class="input71 priceChange" value="<?php echo $info['gap_fee']; ?>" placeholder=""/>
				
				<span class="span24" title="Certification Fee">Service Contract</span>$
				<input name="service_contract" id="service_contract"type="text" class="input71 priceChange" value="" placeholder=""/>
			...
			<div class="left1" style="height: 283px;">
				<div align="center" style="margin-top: 20px;">
                    ...
                    <?php /*?> <a href="#" id="calculateDown" class="btn btn-info pull-left noprint" style="margin-left:10px;">Calulate Down</a><?php */?>
			...
			<div class="right1" style=" top: -25px; position: relative;height: 284px;">
				<table style="width: 80%;">
					...
					<tbody>
						<tr>
							<td width="33%" align="center"><input name="loan_apr" type="text" id="apr1" data-id="1" class="input12 apr" value="" placeholder="" style="width: 83px;"/>
								%</td>
							<td width="33%" align="center"><select name="payment_option1" id="payment_option1" class="form-control price_options" data-id="1" style="width:65%;" price-id="option1_price"><?php foreach($months as $key=>$value){ ?>
			...
			<?php } ?></select><span id="option1_price_span" class="print_month"><?php echo $selected_months[1]; ?></span> months</td>
							<td width="33%" align="center">$&nbsp;
								<input name="option1_price" id="option1_price" type="text" class="input20 options_price" value="" style="width: 80px;"/></td>
						</tr>
						<tr>
							<td width="33%" align="center"><input name="loan_apr2" type="text" id="apr2" data-id="2" class="input12 apr" value="" placeholder="" style="width: 83px;"/>%</td>
							<td width="33%" align="center"><select name="payment_option2" class="form-control price_options" style="width:65%;" id="payment_option2" data-id="1" price-id="option2_price"><?php foreach($months as $key=>$value){ ?>
			...
			<?php } ?></select><span id="option2_price_span" class="print_month"><?php echo $selected_months[2]; ?></span> months</td>
							<td width="33%" align="center">$&nbsp;
								<input name="option2_price" id="option2_price" type="text" class="input20 options_price" value="" style="width: 80px;"/></td>
						</tr>
						<tr>
							<td width="33%" align="center"><input name="loan_apr3" type="text" id="apr3" data-id="3" class="input12 apr" value="" placeholder="" style="width: 83px;"/>%</td>
							<td width="33%" align="center"><select name="payment_option3" class="form-control price_options" style="width:65%;" id="payment_option3" data-id="1" price-id="option3_price"><?php foreach($months as $key=>$value){ ?>
			...
			<?php } ?></select><span id="option3_price_span" class="print_month"><?php echo $selected_months[3]; ?></span> months</td>
							<td width="33%" align="center">$&nbsp;
								<input name="option3_price" id="option3_price" type="text" class="input20 options_price" value="" style="width: 80px;"/></td>
						</tr>
						<?php /*?><tr>
							<td width="33%" align="center"><input name="loan_apr4" type="text" id="apr4" data-id="4" class="input12 apr" value="" placeholder="" style="width: 83px;"/>%</td>
							<td width="33%" align="center"><select name="payment_option4" class="form-control price_options" style="width:65%;" id="payment_option4" price-id="option4_price"><?php foreach($months as $key=>$value){ ?>
			...
			<?php } ?></select><span id="option4_price_span" class="print_month"><?php echo $selected_months[4]; ?></span> months</td>
							<td width="33%" align="center">$&nbsp;
								<input name="option4_price" id="option4_price" type="text" class="input20 options_price" value="" style="width: 80px;"/></td>
						</tr><?php */?>
					</tbody>
...
$(document).ready(function(){
	...
	function calculateProfit()
	{
		...
		{
			gap_fee=($("#gap_fee").val()=='')?0:parseFloat($("#gap_fee").val());
			licreg_fee=($("#licreg_fee").val()=='')?0:parseFloat($("#licreg_fee").val());
			doc=($("#doc").val()=='')?0:parseFloat($("#doc").val());
			profit=parseFloat($("#sell_price").val()-$("#cost").val());
			profit=parseFloat(profit+parseFloat(gap_fee)+parseFloat(licreg_fee)+doc);
		}
	...
	function calculate_tax(){
		...
		var reg_fee = isNaN(parseFloat( $('#licreg_fee').val()))?0.00:parseFloat( $('#licreg_fee').val());
		var gap_fee = isNaN(parseFloat( $('#gap_fee').val()))?0.00:parseFloat( $('#gap_fee').val());
		...
		var tax_amount = isNaN(parseFloat( $("#tax").val()))?0.00:parseFloat( $("#tax").val());
		var tax_percent = (parseFloat(tax_amount)/100)*sell_price;
		
		$("#est_payoff").val(trade_payoff);
		$("#down_pay").val(cash_down);
		actual_balance = (sell_price+trade_payoff+tag_tit+doc+reg_fee)-(trade_allowance + cash_down)+tax_percent+gap_fee+service_contract;
		$('#bal').val( actual_balance.toFixed(2));
		...
		if( $(this).val() != ''){
			$.ajax({
				...
				success: function(data){
					//console.log(data);
					$('#licreg_fee').val(data.licreg_fee);
					//$('#gap_fee').val(data.gap_fee);
	...
	*/
	...
	//calculate();
	if($('#apr1').val() != "" && $('#apr1').val() != null){
	calculateMonthWisePayments(1);
	}else {
...
function calculate() 
{
	...
	var freight = $('#licreg_fee').val();
	var prep = $('#tag_fee').val();
	var doc = $('#doc').val();
	var parts_fee = "0";
	var service_fee = "0";
	...
	//console.log( $('#bal').val() );
	 ...
	 function calculateMonthWisePayments(data_id){
		...
		years=$("#payment_option"+data_id).val()/12;
		pay_monthly=MonthwisePayments(years,data_id);
		price_field=$("#payment_option"+data_id).attr('price-id');										  										  		var payment = document.getElementById(price_field);
	...
	{
		var apr = parseFloat($('#apr'+data_id).val());
