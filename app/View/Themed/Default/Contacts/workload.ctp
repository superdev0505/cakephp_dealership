...
function SelRecordsLimit(RecordSize){
	if(RecordSize!=''){
		var SelTab = $("#hdn_active").val();
		if(SelTab=='tab1'){
			var method = "GetPresaleSection";
			var tabclass = 'presale';
		}else if(SelTab=='tab2'){
			var tabclass = 'postsale';
...
function SelRecordsLimitInSec(RecordSize,Container,URL){
	var user_id = $("#user_id").val();
	$("#"+Container).load( URL+"/"+RecordSize+"/"+user_id);
}
