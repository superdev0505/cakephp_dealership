...
<tr><td colspan="2"><p>This lead has no survey submitted by customer.</p><tr><td>
<?php }else{ ?> 
...
<input type="text" id="email_address" placeholder="Enter Email" /><?php echo $this->Html->link('Email Response',array('controller'=>'email_surveys','action'=>'email_response',$email_survey_id),array('class'=>'btn btn-success no-ajaxify','id'=>'indiv_response')); ?></div>
<br />
		<button type="button" id="btn_print"  class="btn btn-primary">Print Survey</button>
        <?php echo $this->Html->link('Send Response To Group',array('controller'=>'email_surveys','action'=>'email_response',$email_survey_id),array('class'=>'btn btn-success no-ajaxify','id'=>'send_response')); ?>
