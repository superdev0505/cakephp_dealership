...
<?php //if($group == 1 || $group == 2 || $group == 9 || $group == 4 || $group == 10 || $group == 11 || $group == 12 || $group == 13 || $group == 6 ){  ?>
<?php
...
<div class="row">
	<div class="col-md-12">
		...
		<font color="red">*</font> <?php echo $this->Form->label('dealer_id','Select Location:', array('class'=>'strong')); ?>
		<?php
		//debug($dealer_ids);
		...
		<?php
		echo $this->Form->select('user_id', null , array(
		'empty' => 'Select',
		'required' => 'required',
		'selected' => '',
		'class' => 'form-control required_input' ,'style'=>'width: 100%'
...
$(function(){
	...
	$("#ContactDealerId").change(function(){
		if( $(this).val() != ""){
			$.ajax({
				type: "get",
				data: {'dealer_id':$(this).val()},
				url:  "/contacts_transfer/get_user_list/",
				success: function(data){
					$("#ContactUserId").html(data);
				}
			});
		}else{
			$("#ContactUserId").html("");
	...
	$("#btn_move_contact").click(function(event){
		event.preventDefault();
		if( $("#ContactDealerId").val() == ''){
			$("#ContactDealerId").focus()
		...
		<?php if(!empty($this->request->params['named']['action_source']) && $this->request->params['named']['action_source'] == 'bdc_survey_form'){ ?>
				$user_note = $("#ContactNote").val();
				$user_note += ' BDC Marketing Survey Transfer';
				$("#ContactNote").val($user_note);
		<?php } ?>

		$.ajax({
			type: "POST",
			cache: false,
			url:  "/contacts_transfer/move_contact/",
			data: $("#ContactLeadTransferLocationForm").serialize(),
			success: function(data){
				//location.reload();
				<?php if(empty($this->request->params['named']['action_source'])){ ?>
