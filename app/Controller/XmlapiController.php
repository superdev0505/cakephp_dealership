...
class XmlapiController extends AppController {
	...
	public function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('receive_xml','send_xml');
...
//		$history_data['field_changed'] = "";  // for change log
	...
	public function save_xml_file($fileData){
		//Save new file
		$path = WWW_ROOT.'xml';
		$no = 1;
	...
	public function save_lead_to_db($body){
	...
	public function move_xml($filename, $lastid){
		
		$path = WWW_ROOT.'xml';
		$moved_path = WWW_ROOT.'xml_completed';
		rename($path.DS.$filename, $moved_path.DS."{$lastid}_".$filename);
	}

	public function receive_xml(){
		$this->layout = 'ajax';	
		$path = WWW_ROOT.'xml';
		$moved_path = WWW_ROOT.'xml';

		if( $this->request->is('post') && isset($this->request->form['xml_lead'])){

				$new_file = $this->save_xml_file($this->request->form['xml_lead']);
				$new_file_path = $path.DS.$new_file;

				//$new_file = 'D:\wamp\www\app\webroot\xml\xml_lead.xml';	

				$body = file_get_contents($new_file_path);
				//debug($body);
				$result = $this->save_lead_to_db($body);
				echo $result;
				if(!$result === false){
					$this->move_xml($new_file, $result);	
				}
